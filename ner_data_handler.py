...
class NERDataHandler:
  ...
  """
  def __init__(self, model_huggingface_version, batch_size, train_file, val_file = None, test_file = None, sanity_check = False):
    """
    ...
    self.dataset = load_dataset('pandas', data_files = {'train' : train_file, 'val' : val_file, 'test' : test_file})
    tokenized_datasets = self.dataset.map(self.tokenize_and_align_labels, batched=True, remove_columns=self.dataset["train"].column_names)
    data_collator = DataCollatorForTokenClassification(tokenizer=self.tokenizer)
    if sanity_check:
      tokenized_datasets = tokenized_datasets.select(range(100))
    self.train_dataloader = DataLoader(tokenized_datasets["train"], shuffle=True, collate_fn=data_collator, batch_size=batch_size)
    self.val_dataloader = DataLoader(tokenized_datasets["val"], collate_fn=data_collator, batch_size=batch_size)
    self.test_dataloader = DataLoader(tokenized_datasets["test"], collate_fn=data_collator, batch_size=batch_size)
