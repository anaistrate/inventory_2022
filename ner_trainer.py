...
class Trainer():
  ...
  def train(self):
    ...
    for epoch in range(self.num_epochs):
        ...
        # Evaluation
        self.model.eval()
        train_p, train_r, train_f1, _ = self.evaluate(self.train_dataloader)
        ...
        print("Epoch", (epoch + 1),  ": Train Loss: %.5f Precision: %.3f Recall: %.3f F1: %.3f || Val Loss: %.5f Precision: %.3f Recall: %.3f F1: %.3f" % (train_loss, train_p, train_r, train_f1, val_loss, val_p, val_r, val_f1))
  ...
  def postprocess(self, predictions, labels):
    ...
    true_labels = [[ID2NER_TAG[l] for l in label if l != -100] for label in labels]
    pred_labels = [
...
if __name__ == '__main__':
  ...
  parser.add_argument('--test_file', type=str, default = 'data/ner_test.pkl', help = 'Location of test file. Note that it has to be in a .pkl format')
  ...
  data_handler = NERDataHandler(model_huggingface_version, args.batch_size, args.train_file, args.val_file, args.test_file, args.sanity_check)
  ...
  print('=' * 30)
  set_random_seed(45)
  model = AutoModelForTokenClassification.from_pretrained(model_huggingface_version, id2label=ID2NER_TAG, label2id=NER_TAG2ID)
  ...
  checkpt_filename = args.output_dir + 'checkpt_ner_' + args.model_name + '_' + str(best_epoch + 1) + '_epochs'
