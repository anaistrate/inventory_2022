...
class NERPredictor():
  ...
  def __init__(self, model_huggingface_version, checkpoint_filepath):
    ...
    self.model = AutoModelForTokenClassification.from_pretrained(model_huggingface_version, id2label=ID2NER_TAG, label2id=NER_TAG2ID)
  ...
  def predict(self, text):
    ...
    with torch.no_grad():
      ...
      probabilities = torch.nn.functional.softmax(outputs.logits, dim=-1).cpu().numpy()[0][1:-1]
      ...
      for (start, end, word), tag_pred in new_mapping.items():
        ...
        if running_end and tag == 'I-' + running_tag[2:]:
          ...
          if running_start is not None and running_tag != 'O':
            ...
            entry = {'label' : running_tag, 'prob' : running_pred, 'word' : running_word, 'start' : running_start, 'end' : running_end}
...
if __name__ == '__main__':
  ...
  parser.add_argument('--checkpoint-filepath', type=str, default = 'checkpts/checkpt_ner_biomed_roberta_2_epochs', help = 'Location of saved checkpoint file.')
  parser.add_argument('--output_file', type=str, default = 'output_dir/preds_ner.csv', help = 'Output file containing predictions on input_file')
  parser.add_argument('--model-name', type=str, default='biomed_roberta', help = "Name of model to try. Can be one of: ['bert', 'biobert', 'scibert', 'pubmedbert', 'pubmedbert_fulltext', 'bluebert', 'bluebert_mimic3', 'sapbert', 'sapbert_mean_token', 'bioelectra', 'bioelectra_pmc', 'electramed', 'biomed_roberta', 'biomed_roberta_chemprot', 'biomed_roberta_rct500']")
  ...
  # Predict labels
  predictor = NERPredictor(model_huggingface_version, args.checkpoint_filepath)
  all_preds = []
  all_IDs = []
  all_texts = []
  all_probs = []
  ...
  pred_df = pd.DataFrame({'ID' : all_IDs, 'text' : all_texts, 'mention' : all_preds, 'prob': all_probs, 'start_offset': all_offsets_start, 'end_offset' : all_offsets_end})
  print(pred_df[:20])
